...
def main():
    ...
    for testNum in range(numTests):
        ...
        trivium2=Trivium2(key,iv)
        # Determine random number of 32-bit words to encrypt
        numWords = randint(10, 100)
        # 011, 111, 000, 001, 010, 100, 101, 110
        TestData=['fffe0001','b80f09a5','03c0f5bb', '07f20af0','badbe4be', 'baabf4bf', 'ba20c4dc']
        for i in range(7):
            # Generate random 32-bit data word and write to output
            curWord=TestData[i]
            curWord = curWord.zfill(8)
            print(i,'\n','-----------','\n')
            print('Data In:',curWord)
            inFile.write(curWord + "\n")
            temp_data_out=bitListToHex(trivium.encrypt(hexToBitList(curWord))).zfill(8)
            print('Temp Data Out:',temp_data_out)
            temp_data_out=hexToBitList(temp_data_out)
            a=temp_data_out[0]
            b=temp_data_out[1]
            cin=temp_data_out[2]
            result=getResult(a, b, cin) # => (sum,Cout)
            if(result[0]==0 and result[1]==0):
                temp_data_in='fffe0000'
            elif(result[0]==0 and result[1]==1):
                temp_data_in='fffe0001'
            elif(result[0]==1 and result[1]==0):
                temp_data_in='fffe0002'
            else:
                temp_data_in='fffe0003'
            # temp_data_in='fffe000'+str(result[0])+str(result[1])
            print("Temp Data In:",temp_data_in)
            data_out=bitListToHex(trivium2.encrypt(hexToBitList(temp_data_in))).zfill(8)
