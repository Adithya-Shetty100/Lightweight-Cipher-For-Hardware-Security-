...
//                    dat_i <= 32'hfffe0002;
            ...
            6: begin    /* Instruction 6: Get ciphertext from device */
                if (!busy_o) begin
                   ...
                   count=count+1;
                   if(count>1) begin
                   if (dat_o != get_word("trivium_ref_out.txt", dat_cntr_v-1, cur_test_v-1)) begin
                        $display("ERROR: Incorrect output in test %d, word %d!", cur_test_v, dat_cntr_v);
                        $display("Inside");
                        $display("%04x != %04x, input = %04x", dat_o, get_word("trivium_ref_out.txt", dat_cntr_v-1, cur_test_v-1), get_word("trivium_ref_in.txt", dat_cntr_v, cur_test_v));
                        $finish;
                   end
                   end
